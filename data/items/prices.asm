ItemPrices::
	table_width 3, ItemPrices
.Archipelago_Price_Master_Ball
	bcd3 0     ; MASTER_BALL
	bcd3 1200  ; ULTRA_BALL
	bcd3 600   ; GREAT_BALL
	bcd3 200   ; POKE_BALL
	bcd3 3000  ; TOWN_MAP
	bcd3 3000  ; BICYCLE
	bcd3 3000  ; SURFBOARD
	bcd3 1000  ; SAFARI_BALL
	bcd3 3000  ; POKEDEX
	bcd3 2100  ; MOON_STONE
	bcd3 100   ; ANTIDOTE
	bcd3 250   ; BURN_HEAL
	bcd3 250   ; ICE_HEAL
	bcd3 200   ; AWAKENING
	bcd3 200   ; PARLYZ_HEAL
	bcd3 3000  ; FULL_RESTORE
	bcd3 2500  ; MAX_POTION
	bcd3 1500  ; HYPER_POTION
	bcd3 700   ; SUPER_POTION
	bcd3 300   ; POTION
	bcd3 3000  ; BOULDERBADGE
	bcd3 3000  ; CASCADEBADGE
	bcd3 3000  ; THUNDERBADGE
	bcd3 3000  ; RAINBOWBADGE
	bcd3 3000  ; SOULBADGE
	bcd3 3000  ; MARSHBADGE
	bcd3 3000  ; VOLCANOBADGE
	bcd3 3000  ; EARTHBADGE
	bcd3 550   ; ESCAPE_ROPE
	bcd3 350   ; REPEL
	bcd3 3000  ; OLD_AMBER
	bcd3 2100  ; FIRE_STONE
	bcd3 2100  ; THUNDER_STONE
	bcd3 2100  ; WATER_STONE
	bcd3 9800  ; HP_UP
	bcd3 9800  ; PROTEIN
	bcd3 9800  ; IRON
	bcd3 9800  ; CARBOS
	bcd3 9800  ; CALCIUM
	bcd3 4800  ; RARE_CANDY
	bcd3 3000  ; DOME_FOSSIL
	bcd3 3000  ; HELIX_FOSSIL
	bcd3 3000  ; SECRET_KEY
	bcd3 1000  ; AP_ITEM
	bcd3 3000  ; BIKE_VOUCHER
	bcd3 950   ; X_ACCURACY
	bcd3 2100  ; LEAF_STONE
	bcd3 3000  ; CARD_KEY
	bcd3 10000 ; NUGGET
	bcd3 9800  ; XXX PP_UP
	bcd3 1000  ; POKE_DOLL
	bcd3 600   ; FULL_HEAL
	bcd3 1500  ; REVIVE
	bcd3 4000  ; MAX_REVIVE
	bcd3 700   ; GUARD_SPEC
	bcd3 500   ; SUPER_REPEL
	bcd3 700   ; MAX_REPEL
	bcd3 650   ; DIRE_HIT
	bcd3 10    ; COIN
	bcd3 200   ; FRESH_WATER
	bcd3 300   ; SODA_POP
	bcd3 350   ; LEMONADE
	bcd3 3000  ; S_S_TICKET
	bcd3 3000  ; GOLD_TEETH
	bcd3 500   ; X_ATTACK
	bcd3 550   ; X_DEFEND
	bcd3 350   ; X_SPEED
	bcd3 350   ; X_SPECIAL
	bcd3 3000  ; COIN_CASE
	bcd3 3000  ; OAKS_PARCEL
	bcd3 3000  ; ITEMFINDER
	bcd3 3000  ; SILPH_SCOPE
	bcd3 3000  ; POKE_FLUTE
	bcd3 3000  ; LIFT_KEY
	bcd3 3000  ; EXP_ALL
	bcd3 3000  ; OLD_ROD
	bcd3 3000  ; GOOD_ROD
	bcd3 3000  ; SUPER_ROD
	bcd3 0     ; PP_UP
	bcd3 0     ; ETHER
	bcd3 0     ; MAX_ETHER
	bcd3 0     ; ELIXER
	bcd3 0     ; MAX_ELIXER
	bcd3 3000  ; TEA
	bcd3 3000  ; MASTER_SWORD
	bcd3 3000  ; FLUTE
	bcd3 3000  ; TITANS_MITT
	bcd3 3000  ; LAMP
	bcd3 3000  ; PLANT_KEY
	bcd3 3000  ; MANSION_KEY
	bcd3 3000  ; HIDEOUT_KEY
	bcd3 2100  ; TRADE_STONE
	bcd3 3000  ; SAFARI_PASS
	bcd3 100   ; POISON_TRAP
	bcd3 100   ; PARALYZE_TRAP
	bcd3 100   ; ICE_TRAP
	bcd3 100   ; FIRE_TRAP
	bcd3 20    ; TWENTY_COINS
	bcd3 100   ; HUNDRED_COINS
	bcd3 3000  ; CARD_KEY_2F
	bcd3 3000  ; CARD_KEY_3F
	bcd3 3000  ; CARD_KEY_4F
	bcd3 3000  ; CARD_KEY_5F
	bcd3 3000  ; CARD_KEY_6F
	bcd3 3000  ; CARD_KEY_7F
	bcd3 3000  ; CARD_KEY_8F
	bcd3 3000  ; CARD_KEY_9F
	bcd3 3000  ; CARD_KEY_10F
	bcd3 3000  ; CARD_KEY_11F
	bcd3 3000  ; PROGRESSIVE_CARD_KEY
	bcd3 100   ; SLEEP_TRAP
	assert_table_length NUM_ITEMS
	bcd3 0     ; FLOOR_B2F
	bcd3 0     ; FLOOR_B1F
	bcd3 0     ; FLOOR_1F
	bcd3 0     ; FLOOR_2F
	bcd3 0     ; FLOOR_3F
	bcd3 0     ; FLOOR_4F
	bcd3 0     ; FLOOR_5F
	bcd3 0     ; FLOOR_6F
	bcd3 0     ; FLOOR_7F
	bcd3 0     ; FLOOR_8F
	bcd3 0     ; FLOOR_9F
	bcd3 0     ; FLOOR_10F
	bcd3 0     ; FLOOR_11F
	bcd3 0     ; FLOOR_B4F
	assert_table_length NUM_ITEMS + NUM_FLOORS
